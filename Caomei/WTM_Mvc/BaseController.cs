...
namespace Caomei.Mvc
{
    public abstract class BaseController : Controller, IBaseController
    {
        ...
        // rv.FC = new Dictionary<string, object>(); rv.CreatorAssembly =
        // this.GetType().AssemblyQualifiedName; rv.Controller = this; rv.ControllerName =
        // this.GetType().FullName; if (HttpContext != null && HttpContext.Request != null) { try {
        // if (Request.QueryString != QueryString.Empty) { foreach (var key in Request.Query.Keys) {
        // if (rv.FC.Keys.Contains(key) == false) { rv.FC.Add(key, Request.Query[key]); } } } if
        // (HttpContext.Request.HasFormContentType) { var f = HttpContext.Request.Form; foreach (var
        // key in f.Keys) { if (rv.FC.Keys.Contains(key) == false) { rv.FC.Add(key, f[key]); } } } }
        // catch { } } //try to set values to the viewmodel's matching properties if (values !=
        // null) { foreach (var v in values) { PropertyHelper.SetPropertyValue(rv, v.Key, v.Value,
        // null, false); } } //if viewmodel is derrived from BaseCRUDVM<> and Id has value, call
        // ViewModel's GetById method if (Id != null && rv is IBaseCRUDVM<TopBasePoco> cvm) {
        // cvm.SetEntityById(Id); } //if viewmodel is derrived from IBaseBatchVM<>，set ViewMode's
        // Ids property,and init it's ListVM and EditModel properties if (rv is IBaseBatchVM<BaseVM>
        // temp) { temp.Ids = new string[] { }; if (Ids != null) { var tempids = new List<string>();
        // foreach (var iid in Ids) { tempids.Add(iid.ToString()); } temp.Ids = tempids.ToArray(); }
        // if (temp.ListVM != null) { temp.ListVM.CopyContext(rv); temp.ListVM.Ids = Ids == null ?
        // new List<string>() : temp.Ids.ToList(); temp.ListVM.SearcherMode =
        // ListVMSearchModeEnum.Batch; temp.ListVM.NeedPage = false; } if (temp.LinkedVM != null) {
        // temp.LinkedVM.CopyContext(rv); } if (temp.ListVM != null) { //Remove the action columns
        // from list temp.ListVM.OnAfterInitList += (self) => { self.RemoveActionColumn();
        // self.RemoveAction(); if (temp.ErrorMessage.Count > 0) { self.AddErrorColumn(); } };
        // temp.ListVM.DoInitListVM(); if (temp.ListVM.Searcher != null) { var searcher =
        // temp.ListVM.Searcher; searcher.CopyContext(rv); if (passInit == false) {
        // searcher.DoInit(); } } } temp.LinkedVM?.DoInit(); //temp.ListVM.DoSearch(); } //if the
        // viewmodel is a ListVM, Init it's searcher if (rv is IBasePagedListVM<TopBasePoco,
        // ISearcher> lvm) { var searcher = lvm.Searcher; searcher.CopyContext(rv); if (passInit ==
        // false) { searcher.DoInit(); } lvm.DoInitListVM();

        // } if (rv is IBaseImport<BaseTemplateVM> tvm) { var template = tvm.Template;
        // template.CopyContext(rv); template.DoInit(); }
