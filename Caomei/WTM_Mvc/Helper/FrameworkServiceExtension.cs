...
using Caomei.Mvc.Auth;
using Caomei.Mvc.Filters;
using Caomei.Mvc.Helper;
...
namespace Caomei.Mvc
{
    public static class FrameworkServiceExtension
    {
        ...
        {
            var localizer = new ResourceManagerStringLocalizerFactory(Options.Create<LocalizationOptions>(new LocalizationOptions { ResourcesPath = "Resources" }), new Microsoft.Extensions.Logging.LoggerFactory()).Create(typeof(Caomei.Core.CoreProgram));
            var menus = new List<SimpleMenu>();

            if (ConfigInfo.IsQuickDebug)
            {
                menus = new List<SimpleMenu>();
                var areas = allModule.Where(x => x.NameSpace != "Caomei.Admin.Controllers").Select(x => x.Area?.AreaName).Distinct().ToList();
                foreach (var area in areas)
                {
                    ...
                    menus.Add(modelmenu);
                    var cModules = allModule.Where(x => x.NameSpace != "Caomei.Admin.Controllers" && x.Area?.AreaName == area).ToList();
            ...
            builder.AddDataAnnotationsLocalization(options =>
             {
                 options.DataAnnotationLocalizerProvider = (type, factory) =>
                 {
                     ...
                     //{
                     //    return factory.Create(typeof(Caomei.Core.CoreProgram));
                     //}
                     //else
                     //{
        ...
        public static IApplicationBuilder UseWtmContext(this IApplicationBuilder app, bool isspa = false)
        {
            ...
            //获取所有程序集
            //var mvc = GetRuntimeAssembly("Caomei.Mvc");
            //if (mvc != null && gd.AllAssembly.Contains(mvc) == false)
            //{
            //    gd.AllAssembly.Add(mvc);
            //}
            //var core = GetRuntimeAssembly("Caomei.Core");
            ...
            CoreProgram._localizer = programLocalizer;
        ...
        public static IApplicationBuilder UseWtmStaticFiles(this IApplicationBuilder app)
        {
            app.UseStaticFiles(new StaticFileOptions
            {
                ...
                FileProvider = new EmbeddedFileProvider(
                    typeof(CodeGenController).GetTypeInfo().Assembly,
                    "Caomei.Mvc")
            });
            return app;
        }
