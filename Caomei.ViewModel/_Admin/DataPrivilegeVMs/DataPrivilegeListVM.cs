...
namespace WalkingTec.Mvvm.Mvc.Admin.ViewModels.DataPrivilegeVMs
{
    public class DataPrivilegeListVM : BasePagedListVM<DataPrivilege_ListView, DataPrivilegeSearcher>
    {
        protected override List<GridAction> InitGridAction()
        {
            ...
            {
                this.MakeStandardAction("DataPrivilege", GridActionStandardTypesEnum.Create, "","Admin", dialogWidth: 800).SetQueryString($"Type={tp}"),
                this.MakeStandardAction("DataPrivilege", GridActionStandardTypesEnum.ExportExcel, "","Admin"),
            };
        }
        ...
        public List<ColumnFormatInfo> GetOperation(DataPrivilege_ListView item)
        {
            ...
            if (Searcher.DpType == DpTypeEnum.User)
            {
                editurl = "/Admin/DataPrivilege/Edit?ModelName=" + item.TableName + "&Type=User&Id=" + item.TargetId;
                delurl = "/Admin/DataPrivilege/Delete?ModelName=" + item.TableName + "&Type=User&Id=" + item.TargetId;
            }
            else
            {
                editurl = "/Admin/DataPrivilege/Edit?ModelName=" + item.TableName + "&Type=UserGroup&Id=" + item.TargetId;
                delurl = "/Admin/DataPrivilege/Delete?ModelName=" + item.TableName + "&Type=UserGroup&Id=" + item.TargetId;
            }
    ...
    {
        [Display(Name = "Sys.DpTargetName")]
        public string Name { get; set; }

        public string TargetId { get; set; }

        [Display(Name = "Sys.DataPrivilegeName")]
        public string TableName { get; set; }

        [Display(Name = "Sys.DataPrivilegeCount")]
