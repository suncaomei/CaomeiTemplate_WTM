    ...
    public class FrameworkMenuVM2 : BaseCRUDVM<FrameworkMenu>
    {
        ...
        public override void Validate()
        {
            if (Entity.IsInside == true && Entity.FolderOnly == false)
            {
                ...
                {
                    MSD.AddModelError("SelectedModule", Localizer["Validate.{0}required", Localizer["Sys.Module"]]);
                }
                else
                {
                    var modules = Wtm.GlobaInfo.AllModule;
                    ...
                    if (test != null)
                    {
                        MSD.AddModelError(" error", Localizer["Sys.ModuleHasSet"]);
                    }
                }
            }
            base.Validate();
