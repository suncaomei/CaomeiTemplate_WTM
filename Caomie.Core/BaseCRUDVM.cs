    ...
    public class BaseCRUDVM<TModel> : BaseVM, IBaseCRUDVM<TModel> where TModel : TopBasePoco, new()
    {
        ...
        public override void Validate()
        {
            if (ByPassBaseValidation == false)
            {
                ...
                // if (!Validator.TryValidateObject(e, valContext, error, true)) { foreach (var err
                // in error) { string key = err.MemberNames.FirstOrDefault(); if
                // (MSD.Keys.Contains(key) == false) {
        ...
        /// <summary>
        /// 验证重复数据 如果存在重复的数据，则返回已存在数据的id列表 如果不存在重复数据，则返回一个空列表
        /// </summary>
