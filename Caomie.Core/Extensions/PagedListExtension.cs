...
namespace Caomei.Core.Extensions
{
    public static class PagedListExtension
    {
        ...
        // string parameters = "?"; foreach (var item in param) { parameters += item.Key + "=" +
        // item.Value + "&"; } if (self.FromFixedCon == true) { parameters += "DONOTUSECSName=" +
        // self.CurrentCS + "&"; } parameters += "1=1"; if (gridid == null) { gridid = "Grid" +
        // self.UniqueId; } if (MustSelect == false) { excelscript =
        // string.Format("FF_ShowMask(\"{1}\");FF_DownloadExcelOrPdfPost(\"{0}\",\"/WebApi/Home/ExportExcel{2}\");",
        // gridid, "正在生成文件...", parameters); pdfscript =
        // string.Format("FF_ShowMask(\"{1}\");FF_DownloadExcelOrPdfPost(\"{0}\",\"/WebApi/Home/ExportPdf{2}\");",
        // gridid, "正在生成文件...", parameters); } else { excelscript += "var sels = Ext.getCmp(\"" +
        // gridid + "\").getSelectionModel().getSelection();"; excelscript += "if(sels.length == 0){
        // FF_OpenSimpleDialog(\"" + "错误" + "\",\"" + "请至少选择一条数据" + "\");} else{"; excelscript +=
        // string.Format("FF_ShowMask(\"{1}\");FF_DownloadExcelOrPdfPost(\"{0}\",\"/WebApi/Home/ExportExcel{2}\");",
        // gridid, "正在生成文件...", parameters) + "}";
